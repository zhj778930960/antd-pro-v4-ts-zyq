...
import { getAllResourcesAssetPackages } from '@/services/package/resources/index';
...
import { TableColumnsParams } from '@/services/package/resources/index.d';
...
const getAllResourcesAssetPackage = async (params: GetAllParamsType) => {
  ...
  const res = await getAllResourcesAssetPackages(info);
  let result = {
    data: [],
    total: 0,
  };
  if (res?.data) {
    const { asset_packages, total } = res.data;
    result = {
      data: asset_packages,
      total,
    };
...
const Resources: React.FC<{}> = () => {
  ...
  const columns: ProColumnType<TableColumnsParams>[] = [
    ...
    },
    {
      ...
      ellipsis: true,
      render: (_, row) => (
        <Link className={styles.primaryColor} to={`/package/resources/${row.platform}/${row.id}`}>
          {row.name}
        </Link>
      ),
    },
  ...
  return (
    <PageContainer>
      <ProTable
        columns={columns}
        rowKey="id"
        request={(params) => getAllResourcesAssetPackage(params)}
      />
    </PageContainer>
  );
};
