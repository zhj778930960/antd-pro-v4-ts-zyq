...
import { AddTestModule, TestModuleDataType } from '@/services/autotest/module/index.d';
...
// 新增
const addTestModule = async (data: AddTestModule, onCancelModal: () => void) => {
  const res = await addTestModules(data);
  if (res?.status === '00000') {
    notifyInfoTip('测试模块', '新增', true);
    onCancelModal();
  } else {
    notifyInfoTip('测试模块', '新增', false, res.message);
  }
};

// 编辑
const editTestModule = async (
  data: AddTestModule,
  currentRow: TestModuleDataType,
  onCancelModal: () => void,
) => {
  ...
  if (res?.status === '00000') {
    notifyInfoTip('测试模块', '编辑', true);
    onCancelModal();
  } else {
    notifyInfoTip('测试模块', '编辑', false, res.message);
  }
};

const getAllTestModule = async (setUpperModuleData: (data: TreeDataParams[]) => void) => {
...
const CreateParentTest: React.FC<CreateDictionaryPropsType> = (props) => {
  ...
  const modalOk = () => {
    form.validateFields().then((data) => {
      if (modalTitleVisible) {
        addTestModule(data, onCancelModal);
      } else {
        editTestModule(data, currentRow, onCancelModal);
      }
    });
  };

  useEffect(() => {
    getAllTestModule(setUpperModuleData);
    form.resetFields();
...
export default CreateParentTest;
