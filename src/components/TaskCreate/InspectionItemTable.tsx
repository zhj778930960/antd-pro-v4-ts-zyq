...
import { getChilrenItems, delChilrens } from '@/services/inspection/inspection/index';
...
const getChildItems = async (caller: string, currentId?: string) => {
  const result = {
    data: [],
  };
...
const InspectionItemTable: React.FC<{}> = () => {
  const defaultForm = {
    type: '4',
    content_id: [],
  };
  ...
  return (
    <>
      <ProTable
        ...
        request={() => getChildItems(caller, currentId)}
      />
      <InspectionCreate
